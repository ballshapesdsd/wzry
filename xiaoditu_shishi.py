...
while(1):
    ...
    if ret == True:
        ...
        t=heappp(res)
        #print(t)
        #t=heapq.nlargest(300, range(len(res)), res.take)
        
        #右半头像模板匹配
        res1 = cv2.matchTemplate(frame,template1,cv2.TM_SQDIFF_NORMED)
        #print(sorted(res1.reshape(res1.shape[0]*res1.shape[1]))[899])
        sres1=np.copy(res1)
        #res = -res#.reshape(res.shape[0]*res.shape[1])
        #print(res.shape)
        #取最相似的300个坐标
        t1=heappp(res1)
        #t1=heapq.nlargest(300, range(len(res)), res.take)
        
        #下半头像模板匹配
        res2 = cv2.matchTemplate(frame,template2,cv2.TM_SQDIFF_NORMED)
        sres2=np.copy(res2)
        #res = -res#.reshape(res.shape[0]*res.shape[1])
        #取最相似的300个坐标
        t2=heappp(res2)
        #t2=heapq.nlargest(300, range(len(res)), res.take)
        
        #匹配的像素数的最大值
        ma=-1
        
        for temp in t:
            result=matched_pixels_new(temp//349,temp%349,template,frame)
            if result>ma:
                ma=result
                s=sres[temp//349,temp%349]
                #r,g,b=result[1]
                y,x=temp//349,temp%349
        for temp in t1:
            result=matched_pixels_new(temp//365,temp%365-10,template,frame)
            if result>ma:
                ma=result
                #r,g,b=result[1]
                s=sres1[temp//365,temp%365-10]
                y,x=temp//365,temp%365-10
        
        for temp in t2:
            result=matched_pixels_new(temp//349-10,temp%349,template,frame)
            if result>ma:
                ma=result
                #r,g,b=result[1]
                s=sres2[temp//349-10,temp%349]
                y,x=temp//349-10,temp%349
        if ma>470:
            cv2.rectangle(frame, (x,y), (x+32,y+32), 255,2)
        filet.write(str((x,y))+'\n')
        #即时显示位置
        ...
        if k == 27:
            ...
            pass
        print(num-1,time.clock()-start,ma,s)
    else:
        break
